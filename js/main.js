...
// ---- CLIENT COLOR GENERATOR ----
function clientColor(name) {
  var colors = [
    "#4c72ff", "#ff5577", "#33cc88", "#ff9944",
    "#aa55ff", "#00cccc", "#ff4488", "#66bb33",
    "#ff6644", "#4499ff", "#cc44aa", "#88cc00"
  ];
  var hash = 0;
...
function renderClientGrid(clientMap) {
  ...
  if (clients.length === 0) {
    grid.innerHTML = '<div style="color:#333348; padding:30px; text-align:center; font-size:11px;">No clients found.<br>Add items to get started.</div>';
    return;
  }
  ...
  clients.forEach(function (client) {
    ...
    var initial = clientInitial(client);
    ...
    card.innerHTML =
      '<div class="clientInitial" style="background:' + color + '">' + initial + '</div>' +
      '<div class="clientCardInfo">' +
        '<div class="clientCardName">' + client + '</div>' +
        '<div class="clientCardMeta">' + data.total + ' items · ' + typeSummary.join(", ") + '</div>' +
      '</div>' +
      '<div class="clientCardArrow">›</div>';

    card.addEventListener("click", function () {
      openClient(client, color);
    });
...
function updateTabUI() {
  ...
  var shiftHint = document.getElementById("shiftHint");
...
function loadClientLibrary(tab) {
  ...
  var data = document.getElementById("clientViewCount");
  ...
  db.collection(collectionMap[tab]).where("client", "==", currentClient).get()
    .then(function (snapshot) {
      ...
      var total = allClientsData[currentClient] ? allClientsData[currentClient].total : currentData.length;
      data.innerText = currentData.length + " " + tab + " items";

      if (currentData.length === 0) {
        contentEl.innerHTML = '<div style="color:#333348; padding:20px; text-align:center; font-size:11px;">No ' + tab + ' for this client yet.</div>';
        return;
      }
...
document.getElementById("captureBtn").addEventListener("click", function () {
  ...
  csInterface.evalScript(captureCall, function (resultStr) {
    ...
    document.getElementById("output").innerText = "Captured — enter a name and save.";
  });
});
...
document.getElementById("quickCaptureBtn").addEventListener("click", function () {
  ...
  csInterface.evalScript("captureEffects()", function (resultStr) {
    ...
    pendingCapture = result;
    showCaptureForm(result);
    document.getElementById("output").innerText = "Effect captured — enter a name and save.";
  });
});
...
document.getElementById("saveBtn").addEventListener("click", function () {
  ...
  // Client auto-fills from current client context
...
function makeColorHandler(hexValue) {
  return function (e) {
    if (e.shiftKey) {
      csInterface.evalScript('applyStrokeColor("' + hexValue + '")', function (result) {
        document.getElementById("output").innerText = result;
      });
    } else {
      csInterface.evalScript('applyColorSmart("' + hexValue + '")', function (result) {
        document.getElementById("output").innerText = result;
      });
    }
  };
  ...
  return function () {
    ...
    fetch(url + "?t=" + Date.now())
      ...
      .then(function (buffer) {
        var homeFolder = nodeOs.homedir();
        var presetsTemp = homeFolder + "/Documents/DopeTool_Presets";
  ...
  for (var idx = 0; idx < items.length; idx++) {
    ...
    if (tab === "colors") {
      ...
      card.innerHTML =
        ...
        '<div class="cardInfo"><div class="cardTitle">' + data.name + '</div>' +
        '<div class="cardSub">' + (data.font || "") + ' · ' + (data.size || "") + (effectCount > 0 ? ' · ' + effectCount + ' fx' : '') + '</div></div>';
      ...
      card.innerHTML =
        '<div class="cardInfo"><div class="cardTitle">' + data.name + '</div>' +
        '<div class="cardSub">' + (isFFX ? "Animation Preset" : "Captured Effect") + '</div></div>' +
      ...
      card.innerHTML =
        '<div class="cardInfo"><div class="cardTitle">' + data.name + '</div>' +
        '<div class="cardSub">Text Animation</div></div>' +
...
function finishUpdate(newVersion, banner, failed) {
  if (failed.length > 0) {
    banner.innerHTML = '<span>Update failed: ' + failed.join(", ") + '</span>';
  } else {
